...
export class MongoDBTripDAO implements ITripDAO {
  constructor() {
    ...
    console.log('[MongoDBTripDAO] MongoDB connection state:', mongoose.connection.readyState);
  }
  ...
  async save(trip: Trip): Promise<Trip> {
    try {
      ...
      if (existingTrip) {
        ...
        existingTrip.days = trip.days as any;
        existingTrip.budget = trip.budget;
        existingTrip.budgetEstimate = trip.budgetEstimate;
  ...
  async list(userId?: string): Promise<Trip[]> {
    try {
      ...
      const trips: any[] = await TripModel.find(query).sort({ createdAt: -1 });
      ...
      return trips.map((trip: any) => this.mapToTrip(trip));
    } catch (error: any) {
  ...
  private mapToTrip(doc: any): Trip {
    try {
